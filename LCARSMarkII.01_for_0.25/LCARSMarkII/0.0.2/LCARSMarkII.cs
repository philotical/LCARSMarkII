        ...
        public void initObjects()
        {
            ...
            if (lODN == null)
            {
                ...
                ScreenMessages.PostScreenMessage(
                   "<color=#ff9900ff>Initializing ODN Network.</color>",
            ...
            try
            {
                if (LCARS.lODN.ArtefactInventory != null && LCARSNCI_Bridge.Instance.ArtefactInventory_String_SFS_Backup != null)
                {
                    UnityEngine.Debug.Log("### LCARSMarkII initObjects reinstate ArtefactInventory from sfs begin ");
            ...
            catch { UnityEngine.Debug.Log("### LCARSMarkII initObjects LCARSNCI_Bridge.Instance.setShipInventoryFromPersistentFile failed "); }
            ...
            catch { UnityEngine.Debug.Log("### LCARSMarkII initObjects LCARSNCI_Bridge.Instance.setCommunicationQueueFromPersistentFile failed "); }
        ...
        void FixedUpdate()
        {
            ...
            */
            ...
            if (LCARS.lODN.ShipSystems["PropulsionMatrix"].isNominal)
            {
                ...
                // main propulsion power usage
                float performance = lPowSys.draw(lODN.ShipSystems["PropulsionMatrix"].name, LCARS.lODN.ShipStatus.current_total_charge);

                if (LCARS.lODN.ShipStatus.current_LCARS_thrust_x != 0f)
                {
                    grav.AddNewForce_left_right(LCARS.lODN.ShipStatus.current_LCARS_thrust_x * performance, thisVessel);
                }
                if (LCARS.lODN.ShipStatus.current_LCARS_thrust_y != 0f)
                {
                    grav.AddNewForce_ffwd_back(LCARS.lODN.ShipStatus.current_LCARS_thrust_y * performance, thisVessel);
                }
                if (LCARS.lODN.ShipStatus.current_LCARS_thrust_z != 0f)
                {
                    grav.AddNewForce_up_down(LCARS.lODN.ShipStatus.current_LCARS_thrust_z * performance, thisVessel);
                }
            }
