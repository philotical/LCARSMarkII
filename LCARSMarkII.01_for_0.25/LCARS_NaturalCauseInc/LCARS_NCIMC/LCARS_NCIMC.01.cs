...
#region Mission_Assets GUI Top
                    ...
                    if (formtoggle_core_data == false && formtoggle_requirements == false && formtoggle_artefacts == false && formtoggle_SealDeal == false && formtoggle_Personalities == false)
                    {
                        ...
                        if (GUILayout.Button("Requirements"))
                        {
...
*/
                    GUILayout.Label("Select Required NCI-Objects:");
                    //UnityEngine.Debug.Log("### NCIMC createMissionMainGUI 10 ");
...
#region Step Options
                                        ...
                                        if (Step.locationtype == "")
                                        {
                                            ...
                                            if (GUILayout.Button("Distance")) { Step.locationtype = "distance"; }
                                        }
                                        else
                                        {
...
#region Step Conversation Speech
                                            if (mission.steps[story_edit_current_step].Conversation.SpeechList.Count > 0)
                                            {
                                                ...
                                                GUILayout.Label("********************************************************************");
                                                GUILayout.BeginHorizontal();
                                                if (GUILayout.Button("Delete this Speech (no undo)")) 
                                                { 
                                            ...
                                            foreach (LCARS_NCI_Mission_Job J in mission.steps[story_edit_current_step].Jobs.jobList)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (J.jobtype == "")
                                                    {
                                                        ...
                                                        {
                                                            if (GUILayout.Button("Distance")) { J.jobtype = "distance"; }
                                                        }
                                                        if (J.target == "object")
                                                        {
...
#region The Mission Save
        ...
        void SaveMission()
        {
            ...
            DirectoryInfo di = Directory.CreateDirectory(KSPUtil.ApplicationRootPath + "GameData/LCARS_NaturalCauseInc/NCIMissions");
            //Get a binary formatter
            var b = new BinaryFormatter();
            //Create a file
            var f = File.Create(KSPUtil.ApplicationRootPath + "GameData/LCARS_NaturalCauseInc/NCIMissions/" + mission.filename + ".mission_dat");
            //Save the scores
            b.Serialize(f, mission);
            f.Close();
        }

        LCARS_NCI_Mission LoadMission(string fileName)
        {

            UnityEngine.Debug.Log("### NCIMC LoadMission");
            //If not blank then load it
            if (File.Exists(KSPUtil.ApplicationRootPath + "GameData/LCARS_NaturalCauseInc/NCIMissions/" + fileName + ".mission_dat") && fileName.Length > 5)
            {
                UnityEngine.Debug.Log("### NCIMC LoadMission File.Exists = true at " + KSPUtil.ApplicationRootPath + "GameData/LCARS_NaturalCauseInc/NCIMissions/" + fileName + ".mission_dat");
                UnityEngine.Debug.Log("### NCIMC LoadMission fileName.Length = " + fileName.Length);
                //Binary formatter for loading back
                var b = new BinaryFormatter();
                //Get the file
                //var f = File.Open(KSPUtil.ApplicationRootPath + "saves/" + HighLogic.SaveFolder + "/NCI/Personalities.dat", FileMode.Open);
                var f = File.Open(KSPUtil.ApplicationRootPath + "GameData/LCARS_NaturalCauseInc/NCIMissions/" + fileName + ".mission_dat", FileMode.Open);
            ...
            {
                UnityEngine.Debug.Log("### NCIMC LoadMission File.Exists = false at " + KSPUtil.ApplicationRootPath + "GameData/LCARS_NaturalCauseInc/NCIMissions/" + fileName + ".mission_dat");
            }
            return null;
        }
