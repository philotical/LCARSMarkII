...
namespace LCARSMarkII
{
    public class LCARS_Communication_Station
    {
        ...
        bool DecryptButtonClicked = false;
            ...
            foreach (LCARS_CommunicationQueue_Type qT in LCARS.lODN.CommunicationQueue)
            {
                ...
                if (qT.panel_state)
                {
                    ...
                    GUILayout.BeginHorizontal();
                            ...
                            if (qT.EncryptionMode == 0 && !qT.Decrypted)
                            {
                                ...
                                if (LCARS.lODN.ShipSystems["UniversalTranslator"].isNominal)
                                {
                                    if (GUILayout.Button("Tie in the UT"))
                                    {
                                        ...
                                        qT.plain_message = "";
                                        qT.Decrypted = false;
                                        DecryptButtonClicked = true;
                                        qT.Message_Object.decryption_sent_to_log = false;
                                    }
                                }
                                else
                                {
                        ...
                        if (qT.Encrypted && qT.Decrypted && !qT.Message_Object.decryption_sent_to_log && DecryptButtonClicked)
                        {
                            UnityEngine.Debug.Log("### LCARS_Communication_Station getQueueGUI Message Decrypted - send to log now ");
                            string enc_note2 = "";
                            try
                            {
                                enc_note2 = (qT.Message_Object.decryption_artefact.Trim().Length > 0 ) ? " with the Artefact: " + qT.Message_Object.decryption_artefact + "" : "";
                            }
                            catch { }
                            try
                            {
                                LCARSNCI_Bridge.Instance.LCARS_add_to_MissionLog(
                                        "You attempted to decrypt a Message from: " + qT.Message_Object.sender + enc_note2,
                                        "Subject: <" + qT.Message_Object.sender_id_line + ">  Title: <" + qT.Message_Object.title + ">  Message: <" + qT.Message_Object.message + ">"
                                        );
                                qT.Message_Object.decryption_sent_to_log = true;
                                DecryptButtonClicked = false;
                            }
                            catch 
                            {
                                UnityEngine.Debug.Log("### LCARS_Communication_Station getQueueGUI Message Decrypted - send to log now failed ");
                            }
                        }
                        GUILayout.EndVertical();
