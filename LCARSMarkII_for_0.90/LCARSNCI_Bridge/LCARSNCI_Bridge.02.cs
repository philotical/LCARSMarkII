...
namespace LCARSMarkII
{
    ...
    public sealed class LCARSNCI_Bridge
    {
        ...
        {
            Debug.Log("LCARSNCI_Bridge giveArtefactToPlayer begin artefactIDcode=" + artefactIDcode);
    ...
    */
        ...
        public string GetEquippment_Setting(string equippment_code, string setting_name, string goal_id_code = "")
        {
            Debug.Log("LCARSNCI_Bridge GetEquippment_Setting <equippment_code=" + equippment_code + " setting_name=" + setting_name + " goal_id_code=" + goal_id_code + "> begin ");
            if (!LCARS_NCI_BridgeReady()) { return null; }
            LCARS_NCI_Equippment foo = null;
            if (equippment_code != "" && setting_name!="" )
            {
                if (goal_id_code != "" && equippment_code != "IsoFluxDetector")
                {
                    foreach (KeyValuePair<int, LCARS_NCI_Equippment> pair in LCARS_NCI.Instance.Data.MissionArchive.RunningMission.mission.equippment.list)
                    {
                        if (foo != null) { continue; }
                        string artefact_id = pair.Value.artefact1;
                        ...
                        Debug.Log("LCARSNCI_Bridge GetEquippment_Setting <equippment_code=" + equippment_code + " and artefact_id="+artefact_id+"> ");
                        if (E.idcode == equippment_code && (goal_id_code == E.artefact1 || goal_id_code == E.resource))
                        {
                            Debug.Log("LCARSNCI_Bridge GetEquippment_Setting <goal_id_code=" + goal_id_code + " and E.artefact1=" + E.artefact1 + " and E.resource=" + E.resource + "> ");
                            Debug.Log("LCARSNCI_Bridge GetEquippment_Setting <foo found> ");
                            try
                            {
                                foo = LCARS_NCI.Instance.Data.MissionArchive.RunningMission.mission.equippment.list[pair.Key];
                            }
                            catch (Exception ex)
                            {
                ...
                if (equippment_code == "IsoFluxDetector")
                {
                    foreach (KeyValuePair<int, LCARS_NCI_Equippment> pair in LCARS_NCI.Instance.Data.MissionArchive.RunningMission.mission.equippment.list)
                    {
                        if (foo != null) { continue; }
                        LCARS_NCI_Equippment E = pair.Value;
                        if (equippment_code == E.idcode)
                        {
                            try
                            {
                                foo = LCARS_NCI.Instance.Data.MissionArchive.RunningMission.mission.equippment.list[pair.Key];
