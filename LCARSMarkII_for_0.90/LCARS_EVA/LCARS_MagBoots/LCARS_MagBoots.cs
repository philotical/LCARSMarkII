...
*/
                ...
                //JebOut.Animate(AnimationState.Idle);
                if (WeHaveColliderContact)
                {
                    if (Input.GetKey("w"))
                    {
                        ...
                        //JebOut.Animate(AnimationState.Walk);

                        /*
                        foreach (Animation animation in this.vessel.rootPart.FindModelAnimators())
                        { // delivers no result..
                            try { 
                                UnityEngine.Debug.Log("MagBootsController Update:  animation.name=" + animation.name);
                            }
                            catch { }
                            try { 
                                UnityEngine.Debug.Log("MagBootsController Update:  animation.clip=" + animation.clip);
                            }
                            catch { }
                            try
                            {
                                UnityEngine.Debug.Log("MagBootsController Update:  animation.clip.name=" + animation.clip.name);
                            }
                            catch { }
                        }
                        */
