...
namespace LCARSMarkII
{
    class LCARS_Tricorder : PartModule
    {
        ...
        public void ActivateTriCorderAction(KSPActionParam param)
        {
            ActivateTriCorder();
        }

        [KSPAction("DeactivateTriCorder")]
        public void DeactivateTriCorderAction(KSPActionParam param)
        {
            DeactivateTriCorder();
        }
...
*/ 
        ...
        private void TriCorderWindow(int windowID)
        {
            ...
            if (LCARS==null)
            {
                try
                {
                    LCARS = LCARSNCI_Bridge.Instance.getCurrentShipLCARS();
                }
                catch 
                {
                    if (FlightGlobals.ActiveVessel.vesselType != VesselType.EVA)
            ...
            */
             ...
             */
                    ...
                    {
                        TriCorderEnabled = false;
                        Debug.LogError("ImpulseDrive: Tricorder OnTriCorderMenu failed, closing window - ex=" + ex);
                    }
            ...
            */
                    ...
                    {
                        TriCorderEnabled = false;
                        Debug.LogError("ImpulseDrive: Tricorder OnTriCorderWindow failed, closing window - ex=" + ex);
                    }
                    /*
        ...
        public virtual void OnTriCorderWindow()
        {
                ...
                if (LCARS.lODN.ShipSystems.ContainsKey("Iso Flux Detector"))
                {
                    if (LCARS.lODN.IFD_show_results_as_screenmessage && LCARS.lODN.ShipSystems["Iso Flux Detector"].isNominal)
                    {
                        ScreenMessages.PostScreenMessage(
                           "<color=#ff9900ff>(TriCorder) IFD Result: " + LCARS.lODN.IFD_object_distance + " meter</color>",
                          Time.deltaTime, ScreenMessageStyle.UPPER_CENTER
                        );
                    }
                }
            }
            catch { GUILayout.Label("IFD-Failure"); }
            /*
            */
            ...
            try
            {
                ...
                GUILayout.BeginVertical();
                GUILayout.Label("Step: " + LCARSNCI_Bridge.Instance.getCurrentStepID());
                GUILayout.Label("Job: " + LCARSNCI_Bridge.Instance.getCurrentJobID());
                GUILayout.EndVertical();
                GUILayout.BeginVertical();
            }
            catch { GUILayout.Label("NCI-Failure"); }
            /*
            */

            try
            {
                if (LCARS.lODN.IFD_object_of_interest != null)
                {
                    GUILayout.Label("IFD-Target: " + LCARS.lODN.IFD_object_of_interest.name);
                    GUILayout.Label("distance_threshold: " + LCARS.lODN.distance_threshold);
                    if (LCARS.lODN.distance_threshold < LCARS.lODN.IFD_object_distance)
                    {
                        GUILayout.Label("IFD-Result: Object out of range.. ");
                    }
                    else
                    {
                        GUILayout.Label("IFD-Result: " + LCARS.lODN.IFD_object_distance + "");
                    }
                }
                else
                {
            ...
            */
            ...
            catch { GUILayout.Label("Subsystem-Failure"); }
