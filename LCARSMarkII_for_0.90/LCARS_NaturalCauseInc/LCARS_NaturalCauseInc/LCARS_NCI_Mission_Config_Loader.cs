...
namespace LCARSMarkII
{
    class LCARS_NCI_Mission_Config_Loader
    {
        ...
        LCARS_NCI_Mission_Requirement NCIObj;
        public void deconstruct_mission_config_requirements(ConfigNode config)
        {
            ...
            {
                UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Loader deconstruct_mission_config_requirements name=" + Item.GetValue("name"));
                bool isCelestialBody = false;
                try
                {
                    isCelestialBody = (Item.GetValue("isCelestialBody") == "True") ? true : false;
                }
                catch { }
                if (isCelestialBody)
                {
                    NCIObj = new LCARS_NCI_Mission_Requirement();
                    ...
                    NCIObj.isCelestialBody = isCelestialBody;
                    NCIObj.bodyname = Item.GetValue("bodyname");

                    ConfigNode PQSCityObjects = Item.GetNode("PQSCityObjects");
        ...
        public void deconstruct_mission_config_personalities(ConfigNode config)
        {
            ...
            foreach (ConfigNode Item in LIST.GetNodes("PERSONALITY"))
            {
                ...
                try
                {
                    if (!LCARS_NCI.Instance.Data.Personalities.list.ContainsKey(NCIPers.name))
                    {
                        LCARS_NCI_Personality P = new LCARS_NCI_Personality();
                        P.name = NCIPers.name;
                        P.description = NCIPers.description;
                        P.idcode = NCIPers.idcode;
                        P.portrait = NCIPers.portrait;
                        LCARS_NCI.Instance.Data.Personalities.addPersonality(P);
                    }
                }
                catch
                {
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Loader deconstruct_mission_config_personalities failed to add new person to assets NCIPers.name=" + NCIPers.name);
                }
            }
        ...
        public void deconstruct_mission_config_artefacts(ConfigNode config)
        {
            ...
            foreach (ConfigNode Item in LIST.GetNodes("ARTEFACT"))
            {
                ...
                try
                {
                    if (!LCARS_NCI.Instance.Data.Artefacts.list.ContainsKey(NCIArt.idcode))
                    {
                        LCARS_NCI.Instance.Data.Artefacts.list.Add(NCIArt.idcode, NCIArt);
                        UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Loader deconstruct_mission_config_artefacts added new artefact to assets NCIArt.name=" + NCIArt.name);
                    }
                }
                catch
                {
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Loader deconstruct_mission_config_artefacts failed to add new artefact to assets NCIArt.name=" + NCIArt.name);
                }
            }
        ...
        */
        ...
        public void deconstruct_mission_config_steps(ConfigNode config)
        {
            ...
            foreach (ConfigNode Step in STEPS.GetNodes("STEP"))
            {
                ...
                NCIStep.location_body = Step.GetValue("location_body");
                NCIStep.location_PQSCity = Step.GetValue("location_PQSCity");
        ...
        */
        ...
        private LCARS_NCI_Mission_Messages deconstruct_mission_config_steps_step_Messages(ConfigNode Step_Config)
        {
            ...
            foreach (ConfigNode Message_node in MESSAGES.GetNodes("MESSAGE"))
            {
                ...
                Message.requirement_id = LCARS_NCI_Mission_Archive_Tools.ToInt(Message_node.GetValue("requirement_id"));
                Message.NCI_body = Message_node.GetValue("NCI_body");
                Message.NCI_PQSCity = Message_node.GetValue("NCI_PQSCity");
        ...
        private LCARS_NCI_Mission_Step_Jobs deconstruct_mission_config_steps_step_Jobs(ConfigNode Step_Config)
        {
            ...
            foreach (ConfigNode Job_node in JOBLIST.GetNodes("JOB"))
            {
                ...
                Job.requirement_id = LCARS_NCI_Mission_Archive_Tools.ToInt(Job_node.GetValue("requirement_id"));
                Job.NCI_body = Job_node.GetValue("NCI_body");
                Job.NCI_PQSCity = Job_node.GetValue("NCI_PQSCity");
                Job.NCI_object = Job_node.GetValue("NCI_object");
