...
namespace LCARSMarkII
{
    class LCARS_NCI_Mission_Config_Writer
    {
        ...
        public ConfigNode construct_mission_config_requirements(LCARS_NCI_Mission mission)
        {
            ...
            UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 1");
            if (mission.requirements == null)
            {
                mission.requirements = new LCARS_NCI_Mission_Requirement_Manager();
            }
            UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 2");
            if (mission.requirements.list == null)
            {
                mission.requirements.list = new Dictionary<int, LCARS_NCI_Mission_Requirement>();
            }
            UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 3");
            foreach (LCARS_NCI_Mission_Requirement mR in mission.requirements.list.Values)
            {
                UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 4");
                if (mR.isCelestialBody)
                {
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 5");
                    ...
                    cn_requirements_requirement.AddValue("isCelestialBody", mR.isCelestialBody);
                    cn_requirements_requirement.AddValue("bodyname", mR.bodyname);
                    ...
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 6");
                    ConfigNode PQSCityObjects = new ConfigNode("PQSCityObjects");
                    foreach (KeyValuePair<string, List<string>> pair in mR.Locations)
                    {
                        UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 7");
                        ...
                        UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 8");
                        Location.AddValue("name", pair.Key);
                        Location.AddValue("actionpoints", actpoi);
                        PQSCityObjects.AddNode(Location);
                        UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 9");
                    }
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 10");
                    cn_requirements_requirement.AddNode(PQSCityObjects);
                    cn_requirements_list.AddNode(cn_requirements_requirement);
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 11");
                }
                else
                {
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 12");
                    ConfigNode cn_requirements_requirement = new ConfigNode("REQUIREMENT");
                    cn_requirements_requirement.AddValue("id", mR.id);
                    cn_requirements_requirement.AddValue("name", mR.name);
                    cn_requirements_requirement.AddValue("description", mR.description);
                    ...
                    cn_requirements_requirement.AddValue("egg_idcode", mR.egg_idcode);
                    string actionpoints = "";
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 13");
                    if (mR.actionpoints.Count > 0)
                    {
                        actionpoints = mR.actionpoints.Aggregate((i, j) => i + "," + j);
                    }
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 14");
                    cn_requirements_requirement.AddValue("actionpoints", actionpoints);
                    string doors = "";
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 15");
                    if (mR.doors.Count > 0)
                    {
                        doors = mR.doors.Aggregate((i, j) => i + "," + j);
                    }
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 16");
                    cn_requirements_requirement.AddValue("doors", doors);
                    //string GUIWindows = "";
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 17");
                    /*if (mR.GUIWindows.Count > 0)
                    {
                        GUIWindows = mR.GUIWindows.Aggregate((i, j) => i + "," + j);
                    }
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 18");
                    cn_requirements_requirement.AddValue("GUIWindows", GUIWindows);
                    */
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 19");

                    cn_requirements_list.AddNode(cn_requirements_requirement);
                    UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 20");
                }
            }
            UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 21");

            cn_requirements.AddNode(cn_requirements_list);
            UnityEngine.Debug.Log("### NCIMC LCARS_NCI_Mission_Config_Writer construct_mission_config_requirements 22");
        ...
        */
        ...
        public ConfigNode construct_mission_config_steps(LCARS_NCI_Mission mission)
        {
            ...
            foreach (LCARS_NCI_Mission_Step e in mission.steps.Values)
            {
                ...
                cn_steps_step.AddValue("location_body", e.location_body);
                cn_steps_step.AddValue("location_PQSCity", e.location_PQSCity);
        ...
        */
        ...
        private ConfigNode construct_mission_config_steps_step_Messages(LCARS_NCI_Mission_Step Step)
        {
            ...
            foreach(LCARS_NCI_Mission_Message M in Step.Messages.MessageList.Values)
            {
                ...
                cn_Messages_Message.AddValue("requirement_id", M.requirement_id);
                cn_Messages_Message.AddValue("NCI_object", M.NCI_object);
                cn_Messages_Message.AddValue("NCI_egg", M.NCI_egg);
                cn_Messages_Message.AddValue("NCI_body", M.NCI_body);
                cn_Messages_Message.AddValue("NCI_PQSCity", M.NCI_PQSCity);
        ...
        private ConfigNode construct_mission_config_steps_step_Jobs(LCARS_NCI_Mission_Step Step)
        {
            ...
            foreach (LCARS_NCI_Mission_Job M in Step.Jobs.jobList.Values)
            {
                ...
                cn_jobList_Job.AddValue("requirement_id", M.requirement_id);
                cn_jobList_Job.AddValue("NCI_body", M.NCI_body);
                cn_jobList_Job.AddValue("NCI_PQSCity", M.NCI_PQSCity);
